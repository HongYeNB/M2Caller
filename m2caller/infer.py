...
def main():
    ...
    margs = ckpt.get('args', {})
    model = M2CallerModel(**{k:v for k,v in margs.items() if k in ['d_model','layers','heads','ssm_dim']}, vocab_size=len(TOKENS))
    ...
    with torch.no_grad(), open(args.out_fasta, 'w') as fw:
        for read_ids, batch, lengths in tqdm(loader, desc='Infer'):
            ...
            # write one concatenated sequence per original read id
            # since we split windows, we join their decoded strings
            concat = {}
